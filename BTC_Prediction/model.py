...
# --- 3. Training and Evaluation (Keep as is) ---
def train_model(
    model: nn.Module,
...
def load_or_generate_data(
    ...
    # ... (код функции load_or_generate_data из п.2 этого ответа) ...
    try:
        # Put imports here to avoid error if modules not found but cache exists
        import load_data
        import boids
    except ImportError:
        print("Error: Cannot generate data. Ensure load_data.py and boids.py are accessible.")
        pass # Continue to try loading first
    ...
    try:
        ...
        # Use ALL_FEATURE_COLS for dropna check
        ...
        features_to_scale = [col for col in ALL_FEATURE_COLS if col in df_combined.columns] # Ensure we only scale existing cols
        ...
        df_to_save = df_combined[columns_to_save] # Save only the scaled columns needed + target
...
def main():
    ...
    # --- Feature Definition (With Boids) ---
    # Use the globally defined ALL_FEATURE_COLS for consistency
    feature_cols = ALL_FEATURE_COLS
    script_identifier = "with_boids" # For output naming
    ...
    output_dir = results_base_dir / f"{param_str}_{script_identifier}"
    output_dir.mkdir(parents=True, exist_ok=True)
    print(f"Results will be saved to: {output_dir.resolve()}")

    # --- Load or Generate Data ---
    # Pass ALL_FEATURE_COLS to check within loading/generation function
    # Note: load_or_generate_data returns df with ALL features if generated
    ...
    # --- Prepare Sequences ---
    # Create sequences using the features defined for this script (feature_cols)
    ...
    # --- Initialize Model ---
    input_dim = len(feature_cols) # Based on features used by THIS script
    ...
    checkpoint_path = output_dir / f"best_model_{script_identifier}.pth"
    ...
    if best_epoch != -1 and checkpoint_path.is_file():
        best_model = LSTMModel(input_dim, hidden_dim, num_layers)
        try:
            ...
            summary = {
                ...
                "features_used": feature_cols, # Log actual features used
